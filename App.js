...
Bases.forEach(base=>{
  ...
  let Replacment="Math.pow("+base+","; 

  formula_str=formula_str.replace(toReplace,Replacment);
})

const NUMBERS=factorialNumberGetter(data.formula,FACTORIAL_SEARCH_RESULT);

NUMBERS.forEach(factorial=>{
  formula_str=formula_str.replace(factorial.toReplace,factorial.replacment);
})
console.log(NUMBERS);
 let result;
 try{
...
function factorialNumberGetter(formula, FACTORIAL_SEARCH_RESULT)
{
  ...
  FACTORIAL_SEARCH_RESULT.forEach(factorial_index=>{
    ...
    const factorial="factorial(", close_Parenthese=")";
